...
<head>
  ...
  <title>Tambah Produk – OptiStore</title>
...
<div class="max-w-3xl mx-auto p-6">
  <h1 class="text-2xl font-extrabold mb-4">Tambah Produk</h1>
  ...
  <form action="{{ route('products.store') }}" method="POST" class="bg-white p-6 rounded-xl shadow-sm space-y-4">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="block text-sm font-semibold mb-1">Nama *</label>
        <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-xl border-slate-200" required>
      </div>
      <div>
        <label class="block text-sm font-semibold mb-1">SKU</label>
        <input type="text" name="sku" value="{{ old('sku') }}" class="w-full rounded-xl border-slate-200">
      </div>
      ...
      <div>
        ...
        <input type="text" name="brand" value="{{ old('brand') }}" class="w-full rounded-xl border-slate-200">
      </div>

      <div>
        <label class="block text-sm font-semibold mb-1">Harga</label>
        <input type="number" step="0.01" min="0" name="price" value="{{ old('price', 0) }}" class="w-full rounded-xl border-slate-200">
      </div>
      <div>
        <label class="block text-sm font-semibold mb-1">Stok Awal *</label>
        <input type="number" min="0" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" class="w-full rounded-xl border-slate-200" required>
      </div>

      <div>
        <label class="block text-sm font-semibold mb-1">Stok Minimum</label>
        <input type="number" min="0" name="min_stock" value="{{ old('min_stock', 0) }}" class="w-full rounded-xl border-slate-200">
      </div>
    </div>
